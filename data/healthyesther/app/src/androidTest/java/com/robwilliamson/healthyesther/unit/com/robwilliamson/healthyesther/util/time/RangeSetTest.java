...
public class RangeSetTest {
    ...
    private static final Range HUGE_RANGE = new Range(
            FROM.minus(Duration.ofDays(1)),
            TO.plus(Duration.ofDays(1)));
    ...
    public void testNextEdge() {
        ZonedDateTime nextEdge = SMALL_RANGES.getEdgeAfter(
                TO.plus(Duration.ofDays(1)).toInstant());
        assertNull(nextEdge);

        nextEdge = SMALL_RANGES.getEdgeAfter(SMALL_RANGES.to.toInstant());
        assertEquals(SMALL_RANGES.to, nextEdge);

        nextEdge = SMALL_RANGES.getEdgeAfter(TO.toInstant());
        assertEquals(SMALL_RANGES.to, nextEdge);

        nextEdge = SMALL_RANGES.getEdgeAfter(
                SMALL_TO_RANGE.from.minus(Duration.ofSeconds(1)).toInstant());
