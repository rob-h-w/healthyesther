    ...
    implements Serializable
{
    ...
    @Nonnull
    public NoteTable.Row select1(
        @Nonnull
        Database database,
        @Nonnull
        Where where) {
        NoteTable.Row row = select0Or1(database, where);
        if (row == null) {
        ...
        implements Serializable, Key
    {
        ...
        @Override
        public int hashCode() {
            int hash = NoteTable.PrimaryKey.class.getCanonicalName().hashCode();
        ...
        implements Serializable
    {
        ...
        @Nullable
        private com.robwilliamson.healthyesther.db.generated.NoteEventTable.Row[] mNoteEventNoteId;
        ...
        protected void update(
            ...
            final Object note = ((mNote == null)?String.class:mNote);
            NoteTable.PrimaryKey nextPrimaryKey = getNextPrimaryKey();
            int actual = transaction.update("note", getConcretePrimaryKey(), COLUMN_NAMES_FOR_UPDATE, nextPrimaryKey.getId(), mName, note);
            if (actual!= 1) {
        ...
        public void loadAllRelations(Database database) {
            mNoteEventNoteId = HealthDatabase.NOTE_EVENT_TABLE.select(database, WhereContains.foreignKey("note_id", getConcretePrimaryKey().getId()));
        }

        @Nullable
        public com.robwilliamson.healthyesther.db.generated.NoteEventTable.Row[] getNoteEventNoteId() {
            return mNoteEventNoteId;
        }
